...
z2=np.polyfit(x,y,2)
p2 = np.poly1d(z2)
world_df.loc[len(world_df)]=[2070, "World", np.nan]
source = pd.DataFrame({
    'x' : pd.to_datetime(world_df['year'], format='%Y'),
    'y' : p2(world_df["year"])
})

base = alt.Chart(source).mark_line(color="#3498db", strokeWidth=4, interpolate = "natural").encode(
    x=alt.X(
        "x:T",
        title="Year",
...
population_chart = (alt.Chart(population_df)
